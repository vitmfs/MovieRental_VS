 ...
 * The rental class has a method to save, but it is not async, can you make it async and explain to us what is the difference?
    * When we make the method async, we ensure that the save to the database operation completes before returning, while avoiding blocking other request threads. It also improves scability under load.
